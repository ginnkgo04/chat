...
// buildKafkaMessageKey builds a stable session-level key for kafka partitioning.
// User chat A->B and B->A share one key to preserve per-session ordering.
func buildKafkaMessageKey(sendID, receiveID string) string {
	if sendID == "" || receiveID == "" {
		return "chat:unknown"
	}
	...
	for {
		...
		if err != nil {
			...
			if messageMode == "channel" {
				...
				sessionKey := buildKafkaMessageKey(message.SendId, message.ReceiveId)
